    ...
    >
      ...
      <td className="px-6 py-3.5">
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-[var(--text-secondary)] font-bold flex items-center gap-1 uppercase tracking-widest opacity-60">
      ...
      <td className="px-6 py-3.5 text-center">
        <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded border ${data?.days === 'odd' ? 'bg-blue-500/5 border-blue-500/20 text-blue-400' :
        ...
        <div className="flex items-center justify-center gap-1.5">
          {data.computed_status === 'active' && (
            <>
              ...
              <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Faol</span>
            </>
          )}
          {data.computed_status === 'waiting' && (
            <>
              <div className="w-1.5 h-1.5 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]"></div>
              <span className="text-[10px] font-bold text-amber-500 uppercase tracking-widest">Kutilayotgan</span>
            </>
          )}
          {data.computed_status === 'activating_soon' && (
            <>
              <div className="w-1.5 h-1.5 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]"></div>
              <span className="text-[10px] font-bold text-blue-500 uppercase tracking-widest">Yaqinda faol</span>
            </>
          )}
          {data.computed_status === 'inactive' && (
            <>
              <div className="w-1.5 h-1.5 rounded-full bg-red-500 opacity-50"></div>
              <span className="text-[10px] font-bold text-red-500/50 uppercase tracking-widest">Faol emas</span>
            </>
          )}
        </div>
      </td>
