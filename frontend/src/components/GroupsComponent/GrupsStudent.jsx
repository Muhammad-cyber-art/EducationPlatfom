...
export default function GroupsStudent({
  ...
  const handleToggle = async (attendanceId, status, studentId) => {
    if (!groupId) return;

    if (isConfirmMode) {
      onLocalAttendanceChange(studentId, status);
      return;
    }

    try {
      const payload = { is_present: status };
      if (attendanceId) {
      ...
      toast.success(status ? "Keldi" : "Kelmagan", { id: 'attend-success' });
    } catch (error) {
  ...
  return (
    <>
      {students.map((item, index) => {
        ...
        const isPresent = item.id in localOverrides ? localOverrides[item.id] : studentAttendance?.is_present;
        ...
        const renderStatusBadge = () => {
          ...
          return (
            <div className={`inline-flex items-center px-3 py-1.5 rounded-xl border text-[9px] font-black uppercase tracking-tight shadow-sm ${config.bg} ${config.color} ${config.border}`}>
              {config.label}
            </div>
          );
        };
              ...
              <td className="px-4 py-3">
                ...
                >
                  ...
                  <div className="min-w-0">
                    <h4 className="text-[13px] font-black text-[var(--text-primary)] group-hover:text-[var(--gold)] transition-colors uppercase tracking-tight leading-tight whitespace-nowrap overflow-hidden">
                      {item.full_name}
                    </h4>
                    <p className="text-[8px] font-black text-[var(--text-muted)] uppercase tracking-widest mt-0.5 opacity-60">ID:{item.id}</p>
                  </div>
                </div>
              </td>
              ...
              <td className="px-4 py-3 text-center">
                {canEdit ? (
                  <button
                    ...
                    className={`h-9 px-6 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] flex items-center gap-2 transition-all shadow-md active:scale-90 ${isPresent === true
                      ? 'bg-emerald-500 text-black shadow-emerald-500/20'
                      : isPresent === false
                        ? 'bg-red-500 text-white shadow-red-500/20'
                        : 'bg-[var(--bg-void)] text-[var(--text-muted)] border border-[var(--border-glass)]'
                      }`}
                  >
                  ...
                  <div className="flex justify-center">
                    {isPresent === true ? (
                      ...
                      <HelpCircle size={16} className="text-[var(--text-muted)] opacity-30" />
                    )}
                  </div>
                )}
              </td>
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              >
                <div className="flex-1 min-w-0">
                  <h4 className="text-[13px] font-black text-[var(--text-primary)] uppercase tracking-tight leading-tight mb-1">{item.full_name}</h4>
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] text-[var(--text-muted)] font-black uppercase tracking-wider font-mono opacity-60">{item.phone?.slice(-9) || "---"}</span>
                    {renderStatusBadge()}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {canEdit ? (
                  <button
                    onClick={(e) => { e.stopPropagation(); handleToggle(attId, !isPresent, item.id); }}
                    className={`h-9 px-5 rounded-xl flex items-center justify-center gap-1.5 transition-all shadow-md active:scale-95 text-[9px] font-black uppercase tracking-widest ${isPresent === true
                      ? 'bg-emerald-500 text-black shadow-emerald-500/20'
                      : isPresent === false
                        ? 'bg-red-500 text-white shadow-red-500/20'
                        : 'bg-[var(--bg-void)] text-[var(--text-muted)] border border-[var(--border-glass)]'
                  ...
                  >
                    {isPresent === true ? <Check size={14} /> : isPresent === false ? <XIcon size={14} /> : <HelpCircle size={14} />}
                    {typeof isPresent !== 'undefined' && <span>{isPresent ? "KLD" : "YO'Q"}</span>}
                  </button>
                ) : (
                  <div className="flex items-center gap-2">
                    {isPresent === true ? (
