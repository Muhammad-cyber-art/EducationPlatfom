    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 pb-6 border-b border-[var(--border-glass)]">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-4xl font-black text-[var(--text-primary)] tracking-tighter uppercase mb-2 italic flex items-center gap-4">
              Moliya Tarixi
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative group">
            ...
            <input
              ...
              className="lux-input !pl-12 !w-full lg:!w-80 !bg-[var(--bg-void)]/40 !border-[var(--border-glass)] hover:!border-[var(--gold)]/30 focus:!border-[var(--gold)]/50 transition-all placeholder:text-[9px] placeholder:tracking-[0.2em] placeholder:uppercase placeholder:text-[var(--text-muted)]/50"
      ...
      {showFilters && (
        <div className="lux-card !bg-[var(--bg-void)]/80 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 animate-in slide-in-from-top-4 duration-500">
          ...
          <div className="space-y-2">
            <label className="text-[8px] font-black text-[var(--text-muted)] uppercase tracking-widest ml-1 italic group-hover:text-[var(--gold)] transition-colors">Boshlanish Sanasi</label>
            <input
              type="date"
          ...
          <div className="space-y-2">
            <label className="text-[8px] font-black text-[var(--text-muted)] uppercase tracking-widest ml-1 italic">Tugash Sanasi</label>
            <input
              type="date"
          ...
          <div className="flex items-end">
            <button
              ...
              className="w-full h-10 bg-red-500/5 hover:bg-red-500/10 text-red-500 border border-red-500/10 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all"
            >
              Tozalash
            </button>
          </div>
      ...
      <div className="space-y-4">
        <div className="px-10 hidden md:flex items-center justify-between text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.4em] mb-6 italic opacity-30">
        ...
        {isLoading ? (
          <div className="py-40 flex flex-col items-center justify-center gap-6">
            ...
            <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.4em] italic">Accessing Ledger Data...</p>
          </div>
        ) : transactions.length === 0 ? (
          <div className="lux-card !py-32 flex flex-col items-center justify-center text-[var(--text-muted)] italic opacity-30">
      ...
      <div ref={ref} className="mt-16 flex flex-col items-center justify-center gap-4">
        ...
        {!hasNextPage && transactions.length > 0 && (
          <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.4em] italic opacity-20">
  ...
  return (
    <div className={`lux-card !p-6 flex flex-col justify-between group overflow-hidden relative ${colors[color]}`}>
      ...
      <div className="flex justify-between items-start mb-6">
        ...
        {trend && (
          <div className="px-3 py-1 bg-[var(--bg-void)]/40 border border-[var(--border-glass)] rounded-full text-[10px] font-black italic shadow-inner">
            {trend}
          </div>
        )}
      </div>
      <div>
        <p className="text-[10px] font-black uppercase tracking-[0.2em] mb-2 italic opacity-60">{label}</p>
      ...
      <div className="flex items-center gap-6 w-full md:w-auto">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-3 mb-1.5">
            <h4 className="text-lg font-black text-[var(--text-primary)] uppercase tracking-tight group-hover/row:text-[var(--gold)] transition-colors italic truncate">{trx.title}</h4>
            <div className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-widest border border-[var(--border-glass)] shadow-inner ${isIncome ? 'bg-emerald-500/5 text-emerald-500/60' : 'bg-rose-500/5 text-rose-500/60'
              }`}>
              {trx.category_display}
            </div>
          </div>
          <p className="text-[10px] text-[var(--text-muted)] font-bold italic flex items-center gap-2 truncate opacity-60 capitalize">
      ...
      <div className="flex items-center justify-between md:justify-end w-full md:w-auto md:gap-16 border-t md:border-t-0 border-[var(--border-glass)] pt-5 md:pt-0">
        <div className="space-y-2 text-left md:text-right">
          <div className="flex items-center gap-2 md:justify-end text-[var(--gold)] opacity-40">
            <MapPin size={10} />
            <span className="text-[9px] font-black uppercase tracking-widest">{trx.branch_name || "MARKAZIY FILIAL"}</span>
          </div>
          <div className="flex items-center gap-4 text-[var(--text-muted)] opacity-30 md:justify-end">
            ...
            <span className="text-[8px] font-black uppercase tabular-nums">{new Date(trx.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}</span>
          </div>
        </div>

        <div className="flex items-center gap-6">
          <div className="text-right">
            <p className={`text-xl font-black tabular-nums tracking-tighter md:min-w-32 ${isIncome ? 'text-emerald-400' : 'text-rose-400'} italic`}>
              {isIncome ? '+' : '-'}{formatCurrency(trx.amount)}
            </p>
            <p className="text-[8px] font-black text-[var(--text-muted)] uppercase tracking-widest mt-1 opacity-20">
...
const FilterSelect = ({ label, value, onChange, options }) => (
  <div className="space-y-2 group">
    <label className="text-[8px] font-black text-[var(--text-muted)] uppercase tracking-widest ml-1 italic group-hover:text-[var(--gold)] transition-colors">{label}</label>
    <div className="relative">
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="w-full bg-[var(--bg-void)]/40 border border-[var(--border-glass)] text-[var(--text-primary)] text-[10px] font-black uppercase tracking-widest px-4 py-3 rounded-xl outline-none hover:border-[var(--gold)]/30 focus:border-[var(--gold)]/50 appearance-none transition-all cursor-pointer shadow-inner"
      >
        {options.map(opt => (
