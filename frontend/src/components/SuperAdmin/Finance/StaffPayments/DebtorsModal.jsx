...
import { X, Users, CheckCircle2, AlertCircle, Clock, Crown, HandHeart, DollarSign, Calendar, CreditCard, Phone } from "lucide-react";

const FinancialStatusBadge = memo(({ status, label }) => {
    const statusConfig = {
        'discount': { icon: Crown, color: 'text-purple-500', bg: 'bg-purple-500/10', border: 'border-purple-500/20' },
        'low_income': { icon: HandHeart, color: 'text-amber-500', bg: 'bg-amber-500/10', border: 'border-amber-500/20' },
        'negotiated': { icon: DollarSign, color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/20' },
        'teacher_negotiated': { icon: Crown, color: 'text-cyan-500', bg: 'bg-cyan-500/10', border: 'border-cyan-500/20' },
        'regular': { icon: CheckCircle2, color: 'text-emerald-500', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20' }
    };
    const config = statusConfig[status] || statusConfig['regular'];
    const Icon = config.icon;

    return (
        <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md ${config.bg} ${config.border} border ${config.color}`}>
            <Icon size={10} />
        ...
        <div className="flex flex-col gap-2 p-3 bg-[var(--bg-void)]/40 hover:bg-[var(--bg-void)]/60 border border-[var(--border-glass)] rounded-xl transition-all duration-200 group">
            <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center font-black text-sm flex-shrink-0 ${isPaid ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                            ...
                            {hasRefund && (
                                <span className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter ${refundIgnored ? 'bg-red-500/10 text-red-500 line-through' : 'bg-blue-500/10 text-blue-500'}`}>
            ...
            {hasRefund && (
                <div className={`text-[9px] font-bold px-2 py-1 rounded-md flex items-center gap-2 ${refundIgnored ? 'bg-red-500/5 text-red-500/60' : 'bg-blue-500/5 text-blue-500'}`}>
                    <AlertCircle size={10} />
                    {refundIgnored 
        ...
        <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/60 backdrop-blur-[2px] animate-in fade-in duration-200 p-4">
            <div className="bg-[var(--bg-panel)] border border-[var(--border-glass)] rounded-[1.5rem] w-full max-w-4xl max-h-[85vh] overflow-hidden shadow-2xl flex flex-col">
                ...
                <div className="p-4 sm:p-5 border-b border-[var(--border-glass)] flex items-center justify-between bg-[var(--bg-void)]/30">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-[var(--text-muted)] font-bold uppercase tracking-widest">Guruh O'quvchilari</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    {activeTab === 'debtors' ? (
                        unpaidStudents.length === 0 ? (
                            <div className="py-20 text-center opacity-30">
                                ...
                                <p className="text-[10px] font-black uppercase tracking-[0.2em]">Qarzdorlar yo'q</p>
                            </div>
                        ) : (
                            unpaidStudents.map((st) => (
                                <StudentRow key={st.id} student={st} isPaid={false} formatCurrency={formatCurrency} />
                            ))
                        )
                    ) : (
                        paidStudents.length === 0 ? (
                            <div className="py-20 text-center opacity-30">
                                ...
                                <p className="text-[10px] font-black uppercase tracking-[0.2em]">To'lovlar yo'q</p>
                            </div>
                        ) : (
                            paidStudents.map((st) => (
                                <StudentRow key={st.id} student={st} isPaid={true} formatCurrency={formatCurrency} />
                            ))
                        )
                    )}
                </div>
                ...
                <div className="p-4 border-t border-[var(--border-glass)] bg-[var(--bg-void)]/20 flex items-center justify-between">
                    ...
                    <span className={`text-xl font-black tabular-nums tracking-tighter ${activeTab === 'debtors' ? 'text-amber-500' : 'text-emerald-500'}`}>
                        {activeTab === 'debtors' 
