    ...
    <div className="flex-1 min-h-screen bg-[var(--bg-void)] p-3 md:p-5 text-[var(--text-primary)] animate-lux-fade">
      ...
      <div className="max-w-xl mx-auto flex items-center justify-between mb-8 bg-[var(--bg-panel)] p-4 rounded-2xl border border-[var(--border-glass)] backdrop-blur-md shadow-[0_0_30px_rgba(0,0,0,0.2)]">
        <div className="flex items-center gap-4">
          <GoBackButton />
          <div>
            <h1 className="text-xl font-black text-[var(--text-primary)] tracking-tight leading-tight uppercase">Yangi Filial</h1>
            <p className="text-[10px] text-[var(--gold)] font-black uppercase tracking-[0.3em] opacity-80 mt-1">Tizim kengayishi</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-[var(--bg-panel)]/50 border border-[var(--border-glass)] rounded-[1.5rem] md:rounded-[2rem] p-6 md:p-8 backdrop-blur-sm shadow-xl relative overflow-hidden">
            ...
            <div className="flex items-center gap-2.5 mb-8 border-b border-[var(--border-glass)] pb-4">
              ...
              <h2 className="text-xs font-black text-[var(--text-primary)] uppercase tracking-[0.2em]">Filial ma'lumotlari</h2>
            </div>

            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] ml-1">Filial nomi</label>
                <div className="relative group">
                  ...
                  <input
                    ...
                    required
                    className="w-full pl-12 pr-4 py-3.5 bg-[var(--bg-void)] border border-[var(--border-glass)] rounded-xl text-xs font-bold text-[var(--text-primary)] placeholder-[var(--text-muted)]/50 hover:border-[var(--gold)]/30 focus:border-[var(--gold)]/50 outline-none transition-all shadow-inner uppercase tracking-wide"
                  />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] ml-1">Manzil</label>
                <div className="relative group">
          ...
          <div className="flex justify-end gap-3 pt-4">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="px-8 py-4 border border-[var(--border-glass)] text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)] rounded-xl hover:bg-[var(--bg-panel)] hover:text-[var(--text-primary)] transition-all active:scale-95"
            >
              Bekor qilish
            </button>
            <button
              type="submit"
              disabled={loading}
              className="px-10 py-4 bg-[var(--gold)] hover:bg-[var(--gold)]/90 text-black text-[10px] font-black uppercase tracking-widest rounded-xl transition-all active:scale-95 shadow-[0_0_20px_rgba(184,134,11,0.3)] hover:shadow-[0_0_30px_rgba(184,134,11,0.5)] disabled:opacity-50 flex items-center gap-3"
            >
