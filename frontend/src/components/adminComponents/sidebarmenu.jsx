      ...
      <div className={`lux-sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <div className="mb-12 px-2 flex items-center justify-between">
          <div className="flex items-center gap-4 group cursor-pointer" onClick={() => navigate('/admin')}>
            ...
            <div>
              <h1 className="text-[18px] font-black text-[var(--text-primary)] leading-none tracking-tighter uppercase">
                BOSS <span className="text-[var(--gold)] font-light italic">PRIVATE</span>
              </h1>
              <p className="text-[8px] text-[var(--text-muted)] font-black uppercase tracking-[0.3em] mt-1.5 opacity-60">ADMIN Tizimi</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto pr-2 -mr-2 scrollbar-hide space-y-10">
          ...
          <section>
            <h2 className="text-[9px] font-black text-[var(--gold)] uppercase tracking-[0.4em] px-4 mb-6 opacity-50">Asosiy Bo'limlar</h2>
          ...
          {(userInfo?.accessible_branches?.length > 0) && (
            <section>
              <h2 className="text-[9px] font-black text-[var(--gold)] uppercase tracking-[0.4em] px-4 mb-6 opacity-50">Biriktirilgan Filiallar</h2>
        ...
        <div className="mt-auto pt-8 border-t border-[var(--border-glass)] space-y-4">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-black text-[var(--text-primary)] truncate uppercase group-hover:text-[var(--gold)] transition-colors">{userInfo?.first_name || "Tizimda"}</p>
              <div className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
                <p className="text-[8px] text-[var(--text-muted)] font-black uppercase tracking-widest">Tizimda</p>
              </div>
            </div>
          ...
          <button
            ...
            className="w-full h-12 rounded-2xl bg-red-500/5 hover:bg-red-500/10 border border-red-500/10 hover:border-red-500/30 text-red-500 flex items-center justify-center gap-3 transition-all active:scale-95 group font-black text-[10px] uppercase tracking-[0.2em]"
          >
